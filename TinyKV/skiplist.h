...
template <typename valueType, typename keyType>
void SkipList<valueType, keyType>::PrintSkipList() const {
...
template <typename valueType, typename keyType>
valueType* SkipList<valueType, keyType>::search(keyType key) {
	Node* p = head;
...
template <typename valueType, typename keyType>
void SkipList<valueType, keyType>::Insert(const keyType& key, const valueType& value) {
	// 1.寻找插入位置
...
template <typename valueType, typename keyType>
bool SkipList<valueType, keyType>::erase(keyType key) {
	std::vector<Node*> pathList(maxLevel + 1);
	Node* p = head;
